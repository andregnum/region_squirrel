...
pub fn load_cached_bps_villages() -> anyhow::Result<Vec<BpsVillageRecord>> {
    let villages_dir = Path::new("cache/raw/indonesia/bps/villages");

    let mut villages = Vec::new();

    for entry in fs::read_dir(villages_dir)
        .with_context(|| format!("failed to read {}", villages_dir.display()))?
    {
        let entry =
            entry.with_context(|| format!("failed to read entry in {}", villages_dir.display()))?;
        ...
        let parent_bps_code = path
            .file_stem()
            .and_then(|file_stem| file_stem.to_str())
            .ok_or_else(|| anyhow::anyhow!("invalid village cache file name {}", path.display()))?
            .to_string();

        let records: Vec<BpsRegionRecord> = read_json_file(&path)
            .with_context(|| format!("failed to load cached BPS villages {}", path.display()))?;

        for record in records {
            villages.push(BpsVillageRecord {
                parent_bps_code: parent_bps_code.clone(),
                record,
            });
        }
    ...
    villages.sort_by(|left, right| left.record.kode_bps.cmp(&right.record.kode_bps));
